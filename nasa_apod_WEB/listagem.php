    ...
    <div class="container my-3 ">
        ...
        <section>
            <header class="card-header p-3 mb-3 text-center">
                <h2>As mais recente imagens registradas</h2>
            </header>
            <?php 
                ...
                $contador = 0;
                try {
                    $stmt = $conexao->prepare("SELECT date_apod, explanation_apod, title_apod, url_apod FROM nasa_apod.apod");
                
                        if ($stmt->execute() ) {
                            while ($contador != 10 && $rs = $stmt->fetch(PDO::FETCH_OBJ)) {  
